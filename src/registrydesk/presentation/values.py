...
from registrydesk.domain.contracts import OwnerType
...
def owner_display_name(row: RegistryRow) -> str:
    """Append legal-entity identifiers without altering individual names."""
    if row.owner_type != OwnerType.LEGAL_ENTITY.value:
...
def fraction_decimal(value: Fraction) -> Decimal:
    """Convert an exact fraction to Decimal only at the presentation boundary."""
    return Decimal(value.numerator) / Decimal(value.denominator)


def fixed_two(value: Decimal | Fraction) -> str:
    """Format an area/value with two half-up decimal places."""
...
def smart_decimal(value: Decimal | Fraction, max_places: int = SMART_DECIMAL_PLACES) -> str:
    """Format a decimal compactly without binary-float conversion."""
    decimal_value = fraction_decimal(value) if isinstance(value, Fraction) else value
