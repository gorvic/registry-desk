...
from registrydesk.services.export_values import fixed_two, pdf_export_value
...
class PdfExporter:
    ...
    def export(self, registry: RegistryDetails, field_keys: Iterable[str], path: Path) -> None:
        ...
        data: list[list[Paragraph]] = [
            [Paragraph(escape(EXPORT_FIELDS_BY_KEY[key].header_label), header_style) for key in keys]
        ]
        commands: list[tuple] = []
        row_index = 1
        sequence = 1
        ...
        for section_type, section_title in (
            ("квартира", "КВАРТИРИ"),
            ("приміщення", "НЕЖИТЛОВІ ПРИМІЩЕННЯ"),
        ):
            ...
            commands.extend(
                [
                    ...
                    ("BACKGROUND", (0, row_index), (len(keys) - 1, row_index), colors.HexColor("#E5E5E5")),
                ]
            )
            row_index += 1
    ...
    @staticmethod
    def _style_for(key: str, text_style, center_style, number_style):
